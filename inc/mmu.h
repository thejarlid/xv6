...
#define PTE_DSK     BIT64(9)    /* page on disk */
#define PTE_AVL		BITMASK64(11, 9)
#define PTE_RO   BIT64(11) /* read-only page, used to implement copy-on-write fork */
...
#define PML4_INDEX(va)			\
	((UINTPTR_T(va) >> PML4_SHIFT) & (PTRS_PER_PML4 - 1))

#define PDPT_INDEX(va)			\
	((UINTPTR_T(va) >> PDPT_SHIFT) & (PTRS_PER_PDPT - 1))

#define PD_INDEX(va)			\
	((UINTPTR_T(va) >> PD_SHIFT) & (PTRS_PER_PD - 1))

#define PT_INDEX(va)			\
	((UINTPTR_T(va) >> PT_SHIFT) & (PTRS_PER_PT - 1))

#define PGADDR(l4, l3, d, t, o) ((uint64_t)((l4) << PML4_SHIFT| (l3) << PDPT_SHIFT | (d) << PD_SHIFT | (t) << PT_SHIFT | (o)))
...
#define SEG64(type, base, lim, resv1, dpl) (struct segdesc)    \
...
{ (lim) & 0xffff, (uint)(base) & 0xffff,              \
	...
	(uint)(lim) >> 16, 0, 0, 0, 0, (uint)(base) >> 24 }


struct core_map_entry {
	int pid;       // -1 means it is used by kernel only
	uint64_t va;   // if it is used by kernel only, this field is 0
	int refCount;  // keeps track of the number of processes mapped to the page
    struct core_map_entry *next;
};

struct swap_map_entry {
	int pid;
	uint64_t va;
	int refCount;
	int in_use;
